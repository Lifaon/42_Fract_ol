...
/*   Updated: 2018/03/29 15:41:15 by mlantonn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fract_ol.h"

static int	check_newtwo(t_comp z, t_comp r, t_data *data, int index)
{
	t_comp tmp;

	tmp.re = z.re - r.re;
	tmp.im = z.im - r.im;
	if (comp_abs(tmp) < 0.01)
...
int			newtwo(t_data *data, t_comp z)
{
	...
	while (++i <= data->imax)
	{
		if ((col = check_newtwo(z, r[0], data, i)))
			return (col);
		if ((col = check_newtwo(z, r[1], data, i)))
			return (col);
		if ((col = check_newtwo(z, r[2], data, i)))
			return (col);
		calc(&z);
	}
	return (0);
